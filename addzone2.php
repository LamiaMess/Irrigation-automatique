         ...
         <!-- sidebar menu end-->
             ...
             <section id="main-content">
                 <section class="wrapper">
                     ...
                     <form style="position: relative; margin: 100px" method="GET" action="controller/addZone.php">
           <fieldset>
               <legend style="color: Black;">Ajouter une nouvelle zone</legend>
               <div class="form-group row">
                <div class="form-group col-md-6">
                  <label for="name">Nom :</label>
                  <input type="text" name="nomzone" class="form-control" placeholder="Entrer le nom de zone">
                </div>
              </div>
              <div class="form-group col-md-6">
                                       <label>Type sol :</label>
                                       <select name="idtypesol" class="form-control">
                                         <option value="">Selectionner type du sol</option>
                                         <?php
                                         include_once RACINE . '/service/typeSolService.php'; 
                                         $ps = new typeSolService();
                                         foreach ($ps->findAll() as $v) {
                                          
                                         ?>
                                         <option value="<?php echo $v->getIdtypesol(); ?>"> <?php echo $v->getNomtypesol(); ?></option>
                                         <?php } ?>
                                       </select>
                                     </div>
              ...
              <div class="form-group row">
                <div class="form-group col-md-6">
                  <label for="float">La ferme :</label>
                  <select name="idferme" class="form-control">
                    <option value="">Selectionner le nom de la ferme</option>
                    <?php
                    include_once RACINE . '/service/fermeService.php'; 
                    $ps = new fermeService();
                    foreach ($ps->findAll() as $v) {
                    ?>
                    <option value="<?php echo $v->getIdferme(); ?>"> <?php echo $v->getNomferme(); ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
